...
class DB:
    ...
    def getBook(self, token, book_id):
        ...
        try:
            ...
            dto = BookDto(
                id=str(book['id']), 
                title=book['title'],
                author=book['author'] or 'Unknown',  # book['author_name'] or "Unknown",
                publisher=book['publisher'] or 'Unknown',# book['publisher_name'] or "Unknown",
    ...
    def save_bookmark(self, user_id, book_id):
        ...
        try:
            cursor = conn.cursor()
            cursor.execute("INSERT INTO user_favorites (user_id, work_id) VALUES (%s, %s)", (user_id, int(book_id)))
            conn.commit()
            return True
        except Exception as e:
    ...
    def remove_bookmark(self, user_id, book_id):
        ...
        try:
            cursor = conn.cursor()
            cursor.execute("DELETE FROM user_favorites WHERE user_id = %s AND work_id = %s", (user_id, int(book_id)))
            conn.commit()
            return True
        except Exception as e:
